...
import "./selectOrUploadModel.scss"
...
export default function SelectOrUploadModel({
  ...
  const showUploadModelButton = () => {
    if(modelName === "custom") {
      return (
        <div>
          <div id="uploadModelContainer">
            <span>
              {/* THIS MUST BE A LABEL FOR loadEquineModelFile */}
              <label htmlFor="loadEquineModelFile" className="btn btn-primary">{customUploadText} <FontAwesomeIcon icon={faUpload}/></label> &nbsp;
              <input id="loadEquineModelFile" type="file" accept=".eq" onChange={onChangeUploadModelFile}/>
            </span>
            {uploadModelFile && (
  ...
  return (
    <div className="selectOrUploadModel">
